...
interface UseDailyBonusTotalParams {
  teamId: string | null; // NULL for all entities, or specific team_id
}

interface UseDailyBonusTotalResult {
  total: number | null;
  loading: boolean;
  error: Error | null;
}
...
/**
 * Calculates the date range for daily bonus (7am to 7am window)
...
export function useDailyBonusTotal({ teamId }: UseDailyBonusTotalParams): UseDailyBonusTotalResult {
  ...
  useEffect(() => {
    async function fetchTotal() {
      try {
        ...
        // Call RPC function
        const { data, error: rpcError } = await supabaseRead.rpc('get_daily_bonus_total', {
        ...
        console.error('[useDailyBonusTotal] Error fetching daily bonus total:', err);
        setError(err instanceof Error ? err : new Error('Failed to fetch daily bonus total'));
        setTotal(null);
      } finally {
        setLoading(false);
      }
