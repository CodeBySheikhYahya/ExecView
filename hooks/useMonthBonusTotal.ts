...
interface UseMonthBonusTotalParams {
  teamId: string | null; // null for ALL, specific team_id for ENT
}

interface UseMonthBonusTotalResult {
  total: number | null;
  loading: boolean;
  error: Error | null;
}
...
export function useMonthBonusTotal({ teamId }: UseMonthBonusTotalParams): UseMonthBonusTotalResult {
  ...
  useEffect(() => {
    ...
    async function fetchTotal() {
      ...
      try {
        ...
        const { data, error: rpcError } = await supabaseRead.rpc('get_month_bonus_total', {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err : new Error('Failed to fetch month bonus total'));
          setTotal(null);
        }
      } finally {
