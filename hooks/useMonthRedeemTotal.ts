...
interface UseMonthRedeemTotalParams {
  teamId: string | null; // null for ALL, specific team_id for ENT
}

interface UseMonthRedeemTotalResult {
  total: number | null;
  loading: boolean;
  error: Error | null;
}
...
export function useMonthRedeemTotal({ teamId }: UseMonthRedeemTotalParams): UseMonthRedeemTotalResult {
  ...
  useEffect(() => {
    ...
    async function fetchTotal() {
      ...
      try {
        ...
        const { data, error: rpcError } = await supabaseRead.rpc('get_month_redeem_total', {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err : new Error('Failed to fetch month redeem total'));
          setTotal(null);
        }
      } finally {
