...
interface UseWeekRechargeTotalParams {
  teamId: string | null; // null for ALL, specific team_id for ENT
}

interface UseWeekRechargeTotalResult {
  total: number | null;
  loading: boolean;
  error: Error | null;
}
...
export function useWeekRechargeTotal({ teamId }: UseWeekRechargeTotalParams): UseWeekRechargeTotalResult {
  ...
  useEffect(() => {
    ...
    async function fetchTotal() {
      ...
      try {
        ...
        const { data, error: rpcError } = await supabaseRead.rpc('get_week_recharge_total', {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err : new Error('Failed to fetch week recharge total'));
          setTotal(null);
        }
      } finally {
