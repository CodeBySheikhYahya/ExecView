...
CREATE TABLE public.player_platfrom_usernames (
  ...
  CONSTRAINT player_platfrom_usernames_game_id_fkey FOREIGN KEY (game_id) REFERENCES public.games(id),
...
CREATE TABLE public.recharge_requests (
  ...
  overpaid_amount numeric DEFAULT 0,
...
CREATE TABLE public.recharge_tag_mappings (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  payment_method_id uuid NOT NULL,
  team_id uuid,
  tag_id text NOT NULL,
  ct_type text NOT NULL CHECK (ct_type = ANY (ARRAY['ct'::text, 'pt'::text])),
  is_active boolean DEFAULT true,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  updated_by uuid,
  team_ids ARRAY NOT NULL DEFAULT ARRAY[]::uuid[] CHECK (array_length(team_ids, 1) > 0),
  amount_limit numeric,
  CONSTRAINT recharge_tag_mappings_pkey PRIMARY KEY (id),
  CONSTRAINT recharge_tag_mappings_payment_method_id_fkey FOREIGN KEY (payment_method_id) REFERENCES public.payment_methods(id),
  CONSTRAINT recharge_tag_mappings_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id),
  CONSTRAINT recharge_tag_mappings_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.suggestions_and_updates (
  ...
  launch_status text CHECK (launch_status IS NULL OR (launch_status = ANY (ARRAY['coming_soon'::text, 'launched'::text, 'in_progress'::text, 'cancelled'::text]))),
